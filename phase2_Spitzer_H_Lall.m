...
%              C_2=3.4e4                        %[V-1 m-1 Torr-1] This is the constant B, I have changed its name
    ...
    %2) Greenwald fraction
                ...
                Gr_fraction=0.7;%0.15 ph1; 0.7 ph2                              %This is to scale the Gr_limit==> <=1
...
T_prefill=293;                                     %[K], temperature of the prefill gas (20º, room temperature)
...
I_Passive_VV=sum(I_Passive,2); 
    ...
    %MANUAL PLOTS OF THE RESULTS OF RZIp%%%%%%%
        ...
        %%%I_PASSIVE VERUS TIME
          ...
          %v{
            figure;           
            plot(time_adaptive*1e3,Ip_output/(1e3),'LineWidth', 2), grid on
            ylabel('I_p (kA)')
            xlabel(' time (ms)')
            ...
            figure;
            plot(time_adaptive*1e3,I_Passive_VV/(1e3),'LineWidth', 2), grid on
            xlabel(' time (ms)')
            ylabel('I_{VV} (kA)')
            title('Net eddy current in the VV ph2')
            set(gca,'XLim',[min(time*1e3) max(time*1e3)]);
            set(gca, 'FontSize', 13, 'LineWidth', 0.75); %<- Set properties TFG
            Filename = 'I_VV';
            %Filename= sprintf('%s_Trampdown_%dms',Filename,2*T_ramp_Sol);    
...
MinDelta_VPFoutput = min(Delta_VPFoutput); MaxDelta_IPFoutput = max(Delta_IPFoutput);  %[V/ms]
    ...
    xlabel('t (ms)')
    ylabel(' \DeltaI (A/ms)')
    title(' \Delta(Input currents) ph2')
    ...
    xlabel('t (ms)')
    ylabel('\DeltaI (kA/ms)')
    title('\Delta(plasma current) ph2')
    set(gca,'XLim',[min(time*1e3) max(time*1e3)]);    
    set(gca, 'FontSize', 13, 'LineWidth', 0.75); %<- Set properties TFG    
    Filename = 'Delta_Ip';
    %Filename= sprintf('%s_Trampdown_%dms',Filename,2*T_ramp_Sol);    
    ...
    %%%Store of RZIp outputs for plot along ph1
    %RZIP_S2_16(:,1)=time_adaptive;
    %RZIP_S2_16(:,2)=I_Passive_VV;
    %RZIP_S2_16(:,3)=Ip_output;
    
    RZIP_S2_16.time=time_adaptive;
    RZIP_S2_16.I_VV=I_Passive_VV;
    RZIP_S2_16.Ip=Ip_output;
    save('RZIp_outputs_S2_16','RZIP_S2_16')
    %%
  %{     
...
psi_null_ins_VV=psi_null_interpn(R_in,Z_in);    %contour plot!!!
    %Plot
        ...
        Filename = 'Psi';
        %title(sprintf('Psi  at t=%d ms (iter %d/%d)',time_loop(loop)*1e3,loop,length(time_loop)))
        title(sprintf('Psi at t=%d ms ph2',time_loop(loop)*1e3))        
...
%Plots!
  ...
  %Quiver plot Bpol
        ...
        xlabel('R (m)')
        ylabel('Z (m)')
        title(sprintf('B_{pol}  at t=%d ms (iter %d/%d) ph2',time_loop(loop)*1e3,loop,length(time_loop)))
        %title(sprintf('B_{pol} t=%dms for simu %d',time_loop(loop)*1e3,sen))
  ...
  %Bpol 
        ...
        %instedad of discontinuously between pixels
        [c,h]=contourf(R_in,Z_in,log10(FieldsBreak.VV.Bpol),40,'ShowText','on'); %T
        %[c,h]=contour(R_in,Z_in,10^4*FieldsBreak.VV.Bpol,'ShowText','on'); %Gauss
        ...
        title(sprintf('B_{pol} at t=%d ms ph2',time_loop(loop)*1e3))        
        title('B_\theta at t=0ms ph2')   
        %title(sprintf('B_{pol} t=%dms for simu %d',time_loop(loop)*1e3,sen))
        Filename = 'Bpol';
        %Filename= sprintf('%s_simu_%d',Filename,sen);      
        saveas(gcf, strcat(FigDir,ProjectName,Filename,FigExt)); 
...
%Bphi
        figure; 
        [c,h]=contourf(R_in,Z_in,log10(FieldsBreak.VV.Bphi),10,'ShowText','on');
        %clabel(c,h,'Color','white') %to make the text white  
        hold on, grid on
        hh=plot(vessel);
        set(hh, 'EdgeColor', 'k')
        ...
        title(sprintf('B_phi at t=%d ms ph2',time_loop(loop)*1e3))   
        title('B_\phi at t=0ms ph2')          
        Filename = 'Bphi';
        %Filename= sprintf('%s_simu_%d',Filename,sen);      
        saveas(gcf, strcat(FigDir,ProjectName,Filename,FigExt));         
        print(gcf,strcat(FigDir,ProjectName,Filename,'.eps'),'-depsc2')   
   %Lloyd
     ...
     Lloyd=E(R_in).*FieldsBreak.VV.Bphi./FieldsBreak.VV.Bpol;
         figure;
        [c,h]=contourf(R_in,Z_in,log10(Lloyd),10,'ShowText','on');
        %clabel(c,h,'Color','white') %to make the text white  
        hold on, grid on
        hh=plot(vessel);
        set(hh, 'EdgeColor', 'k')
        ...
        %title(sprintf('Lloyd criteria at t=%d ms ph2',time_loop(loop)*1e3))     
        title('E_\phiB_\phi/B_\theta at t=0ms ph2')
        Filename = 'LLoyd';
        %Filename= sprintf('%s_simu_%d',Filename,sen);    
            saveas(gcf, strcat(FigDir,ProjectName,Filename,FigExt)); 
        print(gcf,strcat(FigDir,ProjectName,Filename,'.eps'),'-depsc2')         
    ...
    figure;
        ...
        colormap(Gamma_II)
        c=colorbar; %colorbar
        ylabel(c, 'E_\phiB_\phi/B_\theta (V/m)');
        view(2) %2D view
        plot([min(r_sensors) min(r_sensors) max(r_sensors) max(r_sensors) min(r_sensors)],...
        [min(z_sensors) max(z_sensors) max(z_sensors) min(z_sensors) min(z_sensors)],'g.--','LineWidth', 2)
        %axis equal
...
% { 
   ...
   IntMethod='Lp';   %Lp faster, Phi for debug!!!      % ''Phi'or 'Lp' to switch the integration mode 
        ...
        n_pnts_insideL=20;                  %100 is the ideal to have good plots of the fields, but the L int failures. 
                    %30 gives relative resol of 3.4%, 20 about 5.2%
   ...
   %%%Storing of the non colliding starting points
     ...
     %%%Contour plots
      %1) L
        ...
        %title(sprintf('L  at t=%d ms (iter %d/%d)',time_loop(loop)*1e3,loop,length(time_loop)))   
        %title(sprintf('L at t=%dms for simu %d',time_loop(loop)*1e3,sen))
        title(sprintf('Non colliding at t=%d ms ph2',time_loop(loop)*1e3))   
        Filename = 'L_noL';                
        saveas(gcf, strcat(FigDir,ProjectName,Filename,FigExt));   
      ...
      %2) Pseudo potential U/Vloop
        ...
        title(sprintf('Pseudo potential at t=%d ms ph2',time_loop(loop)*1e3)) 
...
%%%EXPERIMENTAL, L plot as connection length
%Poley uses empirical formulae for alll the VV. I could do it.
%The thing is that a_eff should be the closest distance to the wall.

for i=1:length(y0)
        distance_Rout=abs(y0(i,1)-VesselRMaxInner);
        distance_Rin=abs(y0(i,1)-VesselRMinInner);
        
        if distance_Rout<=distance_Rin  %closest distance is to Rout
            a_eff_L_emp_all(i)=distance_Rout;
        
        else  %closest distance is to Rin
            a_eff_L_emp_all(i)=distance_Rin;
        end
    
end


L_emp_all=0.25*a_eff_L_emp_all'.*FieldsBreak.interpn.Bphi(y0(:,1),y0(:,2))./sqrt(FieldsBreak.interpn.Br(y0(:,1),y0(:,2)).^2+FieldsBreak.interpn.Bz(y0(:,1),y0(:,2)).^2);
  ...
  figure;
        ...
        plot(y0(:,1),y0(:,2),'.')
        [r,c] = size(tri); %number of triangles there
        trisurf(tri, y0(:,1), y0(:,2),L_emp_all,'FaceAlpha',1,'EdgeColor','none'), shading('interp');
        view(2)
        colorbar
        hold on                     %this is to make the transition between values continuous,                                                        %instedad of discontinuously between pixels
        colormap(Gamma_II)
...
Emin= @(L,p,C1,C2) C2*p./log(C1*p*L); %Min E as Paschen state
        ...
        title('Paschen curve ph2'); %d for numbers
        set(gca, 'FontSize', 13); %<- Set properties TFG
        Filename='Paschen_complete';
        saveas(gcf, strcat(FigDir,ProjectName,Filename,FigExt)); 

%%%Plot TFM        
        p=linspace(1e-5,1e-3,10000); 
        figure;
        %subplot(1,2,1)
        loglog(p,Emin(10,p,C_1(1),C_2(1)),'-','LineWidth', 0.95)
        hold on
        ...
        xlabel('p (Torr)')
        ylabel('E_{min} (V/m)')
        legend('L=10m','L=50m','L=100m')
        %title(sprintf('Paschen curve ph2, Gas=%s',Gas_type(1))); %d for numbers
        title('Paschen curve for Hydrogen'); %d for numbers
        set(gca, 'FontSize', 13); %<- Set properties TFG
        Filename='Paschen_TFM';
        saveas(gcf, strcat(FigDir,ProjectName,Filename,FigExt)); 
        print(gcf,strcat(FigDir,ProjectName,Filename,'.eps'),'-depsc2')        
...
%%%%%%End loop paschen plots%%%%%%%%
  ...
  lambda= @(C1,p) 1./(C1.*p); 
  figure; 
  plot(p_single,lambda(C_1(1),p_single),'LineWidth',2), grid on;
  %hold on;
  %plot(p,lambda(C_1(2),p))
  xlabel('p (Tor)')
  ylabel('\lambda (m)')
  set(gca,'Xscale','log') %para poner el eje x en log   
  title('Mean free path \lambda versus p for Hydrogen')
